...
public class SettingsTableTest extends CQLTester
{
    ...
    public void testEncryptionOverride() throws Throwable
    {
        ...
        config.internode_encryption_options = config.internode_encryption_options.withAlgorithm("SUPERSSL");
        check(pre + "algorithm", "SUPERSSL");

        check(pre + "cipher_suites", "[]");
        config.internode_encryption_options = config.internode_encryption_options.withCipherSuites("c1", "c2");
        check(pre + "cipher_suites", "[c1, c2]");

        check(pre + "protocol", config.internode_encryption_options.protocol);
        config.internode_encryption_options = config.internode_encryption_options.withProtocol("TLSv5");
        check(pre + "protocol", "TLSv5");

        check(pre + "optional", "true");
        config.internode_encryption_options = config.internode_encryption_options.withOptional(false);
        check(pre + "optional", "false");

        check(pre + "client_auth", "false");
        config.internode_encryption_options = config.internode_encryption_options.withRequireClientAuth(true);
        check(pre + "client_auth", "true");

        check(pre + "endpoint_verification", "false");
        config.internode_encryption_options = config.internode_encryption_options.withRequireEndpointVerification(true);
        check(pre + "endpoint_verification", "true");

        check(pre + "internode_encryption_options", "none");
        config.internode_encryption_options = config.internode_encryption_options.withInternodeEncryption(InternodeEncryption.all);
        check(pre + "internode_encryption_options", "all");
        check(pre + "enabled", "true");

        check(pre + "legacy_ssl_storage_port", "false");
        config.internode_encryption_options = config.internode_encryption_options.withLegacySslStoragePort(true);
