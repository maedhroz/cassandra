...
public class TransactionStatementTest
{
    ...
    public void updateVariableSubstitutionTest()
    {
        ...
        regularOps.add(new TxnReferenceOperation(TxnReferenceOperation.Kind.Setter, column(TABLE1, "v"), null, new Substitution(reference(user("row2"), TABLE2, "v"))));
    ...
    public void additionAssignmentTest()
    {
        ...
        String query = "BEGIN TRANSACTION\n" +
                       "  SELECT v FROM ks.tbl1 WHERE k=1 AND c=2;\n" +
                       "  UPDATE ks.tbl1 SET v+=1 WHERE k=1 AND c=2;\n" +
                       // TODO: Re-enable when -= is supported...
                       //"  UPDATE ks.tbl2 SET v-=1 WHERE k=2 AND c=2;\n" +
                       "COMMIT TRANSACTION";

        List<TxnReferenceOperation> row1Values = new ArrayList<>();
        row1Values.add(new TxnReferenceOperation(TxnReferenceOperation.Kind.Adder, column(TABLE1, "v"), null, new Constant(ByteBufferUtil.bytes(1))));
...
//        TxnReferenceOperations row2Ops = new TxnReferenceOperations(TABLE2, Clustering.make(bytes(2)), row2Values, Collections.emptyList());
        ...
        Txn expected = TxnBuilder.builder()
                                 .withRead(autoRead(TABLE1, 1), "SELECT * FROM ks.tbl1 WHERE k=1 AND c=2 LIMIT 1")
                                 //.withRead(autoRead(TABLE2, 2), "SELECT * FROM ks.tbl2 WHERE k=2 AND c=2 LIMIT 1")
                                 .withRead(TxnDataName.returning(), "SELECT v FROM ks.tbl1 WHERE k=1 AND c=2")
    ...
    public void testListSubstitution()
    {
        ...
        partitionOps.add(new TxnReferenceOperation(TxnReferenceOperation.Kind.Setter, column(TABLE4, "int_list"), null, new Substitution(reference(user("row1"), TABLE4, "int_list"))));
