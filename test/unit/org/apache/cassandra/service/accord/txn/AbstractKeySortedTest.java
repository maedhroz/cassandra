...
public class AbstractKeySortedTest
{
    ...
    private static PartitionKey key(TableId tableId, int k)
    {
        DecoratedKey dk = ByteOrderedPartitioner.instance.decorateKey(ByteBufferUtil.bytes(k));
        return new PartitionKey(tableId, dk);
    }
    ...
    public void checkIterationForKey()
    {
        ...
        source.forEachForKey(key(0), i -> Assert.fail());
        source.forEachForKey(key(1), i -> Assert.assertEquals(item(1, 5), i));
        source.forEachForKey(key(2), i -> Assert.fail());
        List<Item> actual = new ArrayList<>();
        source.forEachForKey(key(3), actual::add);
        Assert.assertEquals(itemList(item(3, 1), item(3, 3)), actual);
        source.forEachForKey(key(4), i -> Assert.fail());
        source.forEachForKey(key(5), i -> Assert.assertEquals(item(5, 4), i));
        source.forEachForKey(key(6), i -> Assert.assertEquals(item(6, 5), i));
        source.forEachForKey(key(7), i -> Assert.fail());
    }
}
