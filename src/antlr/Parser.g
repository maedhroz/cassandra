...
@members {
    ...
    public ColumnReference.Raw newColumnReference(Selectable.RawIdentifier tuple, Selectable.Raw selectable)
    {
        if (!isParsingTxn)
        ...
        ColumnReference.Raw reference = ColumnReference.Raw.fromSelectable(tuple, selectable);
...
insertValue[List<Object> values]
    : t=term { values.add(t); }
    | {isParsingTxn}? r=columnReference { values.add(new ReferenceValue.Substitution.Raw(r)); }
    ;
...
columnReference returns [ColumnReference.Raw rawRef]
    @init { Selectable.RawIdentifier tuple = null; Selectable.Raw selectable = null; }
    @after { $rawRef = newColumnReference(tuple, selectable); }
    : t=sident ('.' s=referenceSelection)? { tuple = t; selectable = s; }
    ;

referenceSelection returns [Selectable.Raw s]
    : g=refereceSelectionWithoutField m=selectorModifier[g] {$s = m;}
    ;

refereceSelectionWithoutField returns [Selectable.Raw s]
    @init { Selectable.Raw tmp = null; }
    @after { $s = tmp; }
    : sn=sident  { tmp=sn; }
    | (selectionTypeHint)=> h=selectionTypeHint { tmp=h; }
...
normalColumnOperation[UpdateStatement.OperationCollector operations, ColumnIdentifier key]
    ...
    | c=cident i=INTEGER
     ...
     | {isParsingTxn}? r=columnReference
       {
           addRawReferenceOperation(operations, key, new ReferenceOperation.Assignment.Raw(new Operation.SetValue(r), key, new ReferenceValue.Substitution.Raw(r)));
       }
    ;
    ...
    : sig=('+=' | '-=')
      (
          t=term
          {
              if (isParsingTxn)
              {
                  // TODO: What actually fails if this branch is removed?
          ...
          | {isParsingTxn}? c=columnReference
            {
    ...
    : '='
      (
          ...
          | {isParsingTxn}? c=columnReference
            {
                ReferenceValue.Raw right = new ReferenceValue.Substitution.Raw(c);
                addRawReferenceOperation(operations, key, new ReferenceOperation.Assignment.Raw(new Operation.SetElement(k, c), key, right));
            }
      )
    ;
