...
public class Dispatcher
{
    ...
    static Message.Response processRequest(ServerConnection connection, Message.Request request, Overload backpressure)
    {
        ...
        {
            NoSpamLogger.log(logger, NoSpamLogger.Level.INFO, 1, TimeUnit.MINUTES,
                             "Request breached limit(s) on bytes in flight (Endpoint: {}, Global: {}) and triggered backpressure.",
                             ClientResourceLimits.getEndpointLimit(), ClientResourceLimits.getGlobalLimit());
            
            ClientWarn.instance.warn(String.format("Request breached limit(s) on bytes in flight (Endpoint: %d, Global: %d) and triggered backpressure.",
                                                   ClientResourceLimits.getEndpointLimit(), ClientResourceLimits.getGlobalLimit()));
        }
