...
public class ConditionStatement
{
    ...
    public ConditionStatement(ColumnReference reference, Kind kind, Term value)
    {
    ...
    public static class Raw
    {
        ...
        public ConditionStatement prepare(String keyspace, VariableSpecifications bindVariables)
        {
            ...
            if (value != null)
            {
                ...
                // TODO: Any better way to do this (or deduplicate w/ Operation.SetElement)?
                if (value instanceof Constants.Literal && receiver.type.isCollection())
                {
                    switch (((CollectionType<?>)receiver.type).kind)
                    {
                        case LIST:
                            receiver = Lists.indexSpecOf(receiver);
                            break;
                        case MAP:
                            receiver = Maps.keySpecOf(receiver);
                            break;
                        case SET:
