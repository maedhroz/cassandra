...
public class ConditionStatement
{
    ...
    public TxnCondition createCondition(QueryOptions options)
    {
        switch (kind)
        {
            case IS_NOT_NULL:
            case IS_NULL:
                return new TxnCondition.Exists(reference.toValueReference(options), kind.toTxnKind());
            case EQ:
            case NEQ:
            case GT:
            case GTE:
            case LT:
            case LTE:
                return new TxnCondition.Value(reference.toValueReference(options), kind.toTxnKind(), value.bindAndGet(options));
            default:
