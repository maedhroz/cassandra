...
public class TransactionStatement implements CQLStatement
{
    ...
    // TODO: Too much going on in this method. Simplify or factor out.
    @Override
        ...
        if (returningReferences != null)
        {
            ...
            for (ColumnReference reference : returningReferences)
            {
                ...
                if (reference.column.isPartitionKey())
                    result.add(txnReference.getPartitionKey(data));
                else if (reference.column.isClusteringColumn())
                    result.add(txnReference.getClusteringKey(data));
                else
                {
                    ColumnData columnData = txnReference.getColumnData(data);

                    if (reference.isElementSelection())
                    {
                        // TODO: Simplify this so we don't reach into the value reference twice.
                        if (columnData.column().type.isFrozenCollection())
                        {
                            Cell<?> collectionCell = (Cell<?>) columnData;
                            ByteBuffer element = txnReference.getFrozenCollectionElement(collectionCell);
                            result.add(element);
                        }
                        else
                        {
                            if (columnData.column().type instanceof SetType)
                                result.add(((Cell<?>) columnData).path().get(0));
                            else if (columnData.column().type instanceof MapType)
                                result.add(((Cell<?>) columnData).buffer());
                        }
                    }
                    else
