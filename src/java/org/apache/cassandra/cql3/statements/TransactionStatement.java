...
public class TransactionStatement implements CQLStatement
{
    ...
    TxnRead createRead(QueryOptions options, Consumer<Key<AccordKey>> keyConsumer)
    {
    ...
    List<TxnWrite.Fragment> createWriteFragments(QueryOptions options, Consumer<Key<AccordKey>> keyConsumer)
    {
    ...
    TxnUpdate createUpdate(QueryOptions options, Consumer<Key<AccordKey>> keyConsumer)
    {
        return new TxnUpdate(createWriteFragments(options, keyConsumer), createCondition(options));
    }

    Keys toKeys(Set<Key<AccordKey>> keySet)
    {
        @SuppressWarnings("unchecked") Key<AccordKey>[] keyArray = new Key[keySet.size()];
    ...
    {
        Set<Key<AccordKey>> keySet = new HashSet<>();
    ...
    {
        TxnData data = StorageProxy.instance.txn(createTxn(options));
